...
DEEPLAB_DIR=/home/gpu_user/assia/ws/tf/deeplab/
DATA_ROOT_DIR=/home/gpu_user/assia/ws/datasets/

DATASET=voc12
LOAD_MAT_FILE=1
MODEL_NAME=deep_largeFOV
...
#######################################
		    CRF_DIR="${DEEPLAB_DIR}"/code/densecrf
		    ...
		    if [ ${DATASET} == "voc12" ]
		    then
          IMG_DIR_NAME="${DATA_ROOT_DIR}"VOC2012_orig
        ...
        # FEATURE_DIR saves the features for those "num_sample" images		    
		    FEATURE_DIR="${DEEPLAB_DIR}"/exper/${DATASET}/features/${MODEL_NAME}/${TEST_SET}/${FEATURE_TYPE}/${FILE_NAME}_numSample${NUM_SAMPLE}
		    
		    SAVE_DIR="${DEEPLAB_DIR}"/exper/${DATASET}/res/features/${MODEL_NAME}/${TEST_SET}/${FEATURE_TYPE}/post_densecrf_W${Bi_W}_XStd${Bi_X_STD}_RStd${Bi_R_STD}_PosW${POS_W}_PosXStd${POS_X_STD}_numSample${NUM_SAMPLE}
